...
import { listProducts, productFormOptions } from "../_lib/data";
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string }> }) {
  const user=await requireAdmin("products.view","/admin/products"); const {q="",category=""}=await searchParams; const [products,options]=await Promise.all([listProducts(q,category),productFormOptions()]);
  return <div className="admin-page"><div className="admin-heading"><div><p className="admin-kicker">CATALOGUE</p><h1>Products</h1><p>Create, review and publish catalogue records.</p></div></div>{can(user,"products.create")&&<AddProductPanel options={options}/>} 
    <form className="admin-filter"><label htmlFor="product-search">Search products</label><div><input id="product-search" name="q" defaultValue={q} placeholder="Name, bearing number or SKU"/><select name="category" defaultValue={category} aria-label="Filter by category"><option value="">All categories</option>{options.categories.map(item=><option value={item.slug} key={item.slug}>{item.name}</option>)}</select><button>Search</button>{(q||category)&&<Link href="/admin/products">Clear</Link>}</div></form>
